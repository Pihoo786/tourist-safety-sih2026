  ...
  const register = async (e) => {
   ...
   try {
     const payload = {
       ...
       expected_exit_date: form.expected_exit_date
     }

     const data = await api('/api/tourists', {
       method: 'POST',
       headers: {
     ...
     setNotice(
       `Digital ID ${data.tourist_id} is now ACTIVE.`
     )
...
function IncidentLine({incident,onClick}) { return <button className="incident-line" onClick={onClick}><span className={incident.incident_type==='SOS'?'sos-icon':'risk-icon'}>{incident.incident_type==='SOS'?'!':'▲'}</span><div><b>{incident.incident_type} · {incident.tourist?.tourist_id || 'Unknown'}</b><small>{incident.tourist?.name || 'Unknown'} · {incident.status}</small></div><time>{new Date(incident.created_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</time></button> }
function DigitalCard({ tourist }) {
  return (
    <div className="digital-card">
      <div className="digital-card-title">
        TOURIST SHIELD · DIGITAL ID
      </div>
      ...
      <div className="digital-card-main">
        <div>
          ...
          <span className="digital-active">
            ACTIVE
          </span>

          <p>
            Valid until {tourist.expected_exit_date}
          </p>
        </div>
        ...
        {tourist.qr_code ? (
          ...
          <div className="digital-qr-missing">
            QR unavailable
          </div>
        )}
      </div>
